...
async function main() {
  ...
  console.log('Lookups:', Object.keys(regionLookup).length, 'regions,', Object.keys(charityLookup).length, 'charities,', Object.keys(advisorLookup).length, 'advisors');
  
  // Import all orders (skip status and class_type for now - they need valid options)
  ...
  for (let i = 0; i < allOrders.length; i += 10) {
    ...
    process.stdout.write(`\r  Progress: ${i + batch.length}/${allOrders.length} (created: ${created})`);
    await new Promise(r => setTimeout(r, 200));
  }
  
  console.log(`\n\nDone! Created ${created} orders (${errors} batches had errors)`);
}
